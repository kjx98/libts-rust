...
use super::super::serde::{Result, Error};
//use std::fmt;
...
pub fn from_bytes<'a>(buf: &'a [u8]) -> Result<Message<'a>> {
    if buf.len() < 8 { return Err(Error::Eof); }
    match buf[0] {
    b'S' => {
...
pub fn to_bytes<'a>(v: &'a Message) -> Result<Vec<u8>> {
    ...
    Body::SystemEvent(s) => {
        ...
        let src = SystemEventNet { tag, event_code, index, tracking, timestamp, time_hours};
        ser_to_bytes(&src)
    },
    Body::SymbolDirectory(s) => {
        let tag = b'R';
        let (market_category, symbol, classification, precision, lot_size, turnover_multi, lower_limit, upper_limit) = (s.market_category, s.symbol, s.classification, s.precision, s.round_lot_size, s.turnover_multi, s.lower_limit, s.upper_limit);
        let src = SymbolDirectoryNet {tag, index, tracking, timestamp, market_category, symbol, classification, precision, lot_size, turnover_multi, lower_limit, upper_limit};
        ser_to_bytes(&src)
    },
    Body::TradingAction(s) => {
        let tag = b'H';
        let (trading_state, reason) = (s.trading_state as u8, s.reason);
        let src = SymbolTradingActionNet {tag, index, tracking, timestamp,trading_state, reason};
        ser_to_bytes(&src)
    },
    Body::AddOrder(s) => {
        let tag = b'A';
        let (buy_sell, ref_no, qty, price) = (s.side as u8, s.reference, s.qty, s.price);
        let src = AddOrderNet {tag, index, tracking, timestamp,buy_sell, ref_no, qty, price};
        ser_to_bytes(&src)
    },
    Body::OrderExecuted(s) => {
        let tag = b'E';
        let (printable, ref_no, qty, match_no) = (s.printable, s.reference, s.qty, s.match_no);
        let src = OrderExecutedNet{ tag, index, tracking, timestamp, printable, ref_no, qty, match_no};
        ser_to_bytes(&src)
    },
    Body::OrderExecutedWithPrice(s) => {
        let tag = b'C';
        let (printable, ref_no, qty, price, match_no) = (s.printable, s.reference, s.qty, s.price, s.match_no);
        let src = OrderExecutedWithPriceNet{ tag, index, tracking, timestamp, printable, ref_no, qty, price, match_no};
        ser_to_bytes(&src)
    },
    Body::OrderCancelled(s) => {
        let tag = b'X';
        let (cancel_reason, ref_no, qty) = (s.reason as u8, s.reference, s.cancelled);
        let src = OrderCancelNet { tag, index, tracking, timestamp, cancel_reason, ref_no, qty};
        ser_to_bytes(&src)
    },
    Body::OrderDelete(s) => {
        let tag = b'D';
        let (cancel_reason, ref_no) = (s.reason as u8, s.reference);
        let src = OrderDeleteNet { tag, index, tracking, timestamp, cancel_reason, ref_no};
        ser_to_bytes(&src)
    },
    Body::ReplaceOrder(s) => {
        let tag = b'U';
        let (ref_no, new_ref_no, qty, price) = (s.old_reference, s.new_reference, s.qty, s.price);
        let src = OrderReplaceNet { tag, index, tracking, timestamp, ref_no, new_ref_no, qty, price };
        ser_to_bytes(&src)
    },
    Body::Trade(s) => {
        let tag = b'P';
        let (buy_sell, ref_no, qty, price, match_no) = (s.side as u8, s.reference, s.qty, s.price, s.match_no);
        let src = TradeNet {tag, index, tracking, timestamp, buy_sell, ref_no, qty, price, match_no};
        ser_to_bytes(&src)
    },
    Body::CrossTrade(s) => {
        let tag = b'Q';
        ...
        let src = CrossTradeNet {tag, index, tracking, timestamp, qty, price, match_no, type_, pclose, open_interest};
        ser_to_bytes(&src)
    },
    }
}
